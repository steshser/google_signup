...
def find_element(driver, locator, waiting_time=10):
    return WebDriverWait(driver, waiting_time).until(EC.presence_of_element_located(locator),
                                                     message=f"Can't find element by locator {locator}")


def find_elements(driver, locator, waiting_time=10):
    return WebDriverWait(driver, waiting_time).until(EC.presence_of_all_elements_located(locator),
                                                     message=f"Can't find elements by locator {locator}")


def is_element_visible(driver, locator, waiting_time=30):
    try:
